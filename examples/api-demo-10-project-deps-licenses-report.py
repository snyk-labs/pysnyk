...
***REMOVED***'--outputPathCSV', type=str,
                        help='The desired output if you want CSV output.')

***REMOVED***'--outputPathNestedJson', type=str,
                        help='The desired output if you want a nested JSON output.')

***REMOVED***'--outputPathFlatJson', type=str,
    ...
    if args.outputPathExcel is None and \
            ...
            args.outputPathNestedJson is None and \
            args.outputPathFlatJson is None:
...
def write_all_projects_nested_json(all_project_info, output_path):
    ...
    projects_obj = {
        'projects': all_projects_with_flattened_dependencies_tree_removed
    }
...
def write_all_project_output_csv(all_project_info, output_path):
    ...
    with open(output_path, 'w') as output_csv:
        for next_project in all_project_info:
            str_csv_line = '%s  %s  %s  %s' % (next_project['project_name'], 'License(s)', 'License Issue(s)', 'Application and Path')
            output_csv.write('%s\n' % str_csv_line)

            flattened_dependencies_list = next_project['flat_deps_list']
            for next_dep in flattened_dependencies_list:
                str_csv_line = '%s  %s  %s  %s' % (next_dep['pkgId'], next_dep['licenses'], next_dep['license_issues'], next_dep['path'])
                output_csv.write('%s\n' % str_csv_line)

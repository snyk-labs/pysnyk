...
class analysisLog:
    ...
    def printCallToAction(self):
        print('\n\n\t\t****Call To Action***')
        print('\t\t\t A fix was found! The fix for the highest severity fixable issue is:')
        print('\t\t\t\tPackage Manager: ' + self.callToAction[3])
        print('\t\t\t\tUpgrade: ' + self.callToAction[4] +  ' to ' + self.callToAction[5])
    ...
    def checkCallToAction(self,curDep):
            ...
            if(self.fixableNewsStoryFound == False):  #check if a news story has been found, if it has, exit, you have your call to action
                if(curDep['cvssScore'] > self.callToAction[0]): #current issue being looked at is more severe than previous found
                    if((curDep['isUpgradable']) or (curDep['isPatchable'])): #check if a fix is available
                        if(curDep['isUpgradable']): #otherwise no upgrade path
                            
                            if(curDep['upgradePath'][0] != curDep['from'][0]): #not a dependency install issue
                    ...
                    if(tmpUpgradePath != ''): #upgrade found
                        ...
                        self.callToAction = [curDep['cvssScore'], curDep['title'], curIsOnWatchlist,curDep['packageManager'],curDep['from'][0] , tmpUpgradePath]
                        self.fixFound=True
    #Analysis worker
...
***REMOVED***
json_rslts_repo = client.snyk_projects_project_issues(org_id, project_id)
